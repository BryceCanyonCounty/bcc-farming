Locales["fr_lang"] = {
    noSoil = "Vous n'avez pas assez de terre pour planter la graine",
    noSeed = "Vous n'avez pas assez de graines pour planter",
    noPlantingTool = "Vous avez besoin d'un outil de plantation pour planter la graine",
    tooCloseToTown = "Vous êtes trop près d'une ville pour planter la graine",
    incorrectJob = "Vous n'êtes pas le bon travailleur pour planter la graine",
    maxPlantsReached = "Vous avez atteint le nombre maximum de plantes",
    raking = "Vous ratissez la terre",
    plantingDone = "Vous avez fini de planter la graine",
    failed = "Vous avez échoué à planter la graine",
    fertilize = "Fertiliser",
    yes = "Oui",
    no = "Non",
    noFert = "Vous n'avez pas assez d'engrais pour fertiliser la graine",
    noWaterBucket = "Vous n'avez pas de seau d'eau pour arroser la graine",
    waterPlant = "Arroser la plante",
    plant = "Planter",
    secondsUntilharvest = "Secondes jusqu'à la récolte : ",
    secondsUntilharvestOver = "Secondes jusqu'à la récolte : Terminé",
    harvest = "Récolter",
    destroyPlant = "Détruire la plante",
    plantNotGrown = "La plante n'a pas encore poussé",
    harvested = "Harvested: ",
    tooCloseToAnotherPlant = "Vous êtes trop près d'une autre plante",
    mustUseLockedSpot = "Vous devez planter sur un emplacement autorisé",
    lockedSpotTaken = "Cet emplacement de plantation est déjà utilisé",
    needHouseOwnership = "Vous devez posséder une maison pour planter",
    needHousePlot = "Vous devez planter sur votre propriété",
    movedTooFar = "Vous vous êtes trop éloigné",
    noCarry = "Can't carry item: ",
    UsageLeft = "Durability: ",
    WaterUsesLeft = "Utilisations d'eau restantes: ",
    FinishPlantingProcessFirst = "Finish growing the current plant first",
    needNewTool = "Tool broken, you need a new one!",
    useBucket = "Use Bucket",
    --- Smelling 
    SmellPlant = "There is the scent of a suspicious plant in the air",
    SmellablePlant = "Smellable plant: ",
}

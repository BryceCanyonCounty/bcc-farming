...
AddEventHandler('bcc-farming:WaterPlantMain', function(plantcoords, v, object, plantid)
    ...
    local firstprompt2 = PromptGroup2:RegisterPrompt(_U('WaterCropPrompt'), 0x760A9C6F, 1, 1, true, 'hold', {timedeventhash = "MEDIUM_TIMED_EVENT"})
    ...
    while true do
        ...
        if dist < 2 then
            PromptGroup2:ShowGroup(_U('WaterCropPrompt'))
...
RegisterNetEvent('bcc-farming:WaterCrop', function(type, blip, v, plantcoords, object, plantid)
    if type == 'water' then
        ...
        VORPcore.NotifyRightTip(_U('CropWatered'), 4000)
        type = 'fert'
    end
    
    if type == 'fert' then
        local PromptGroup2 = BccUtils.Prompts:SetupPromptGroup()
        local firstprompt2 = PromptGroup2:RegisterPrompt(_U('PlantWithFertilizer'), 0x760A9C6F, 1, 1, true, 'hold', {timedeventhash = "MEDIUM_TIMED_EVENT"})
        local firstprompt3 = PromptGroup2:RegisterPrompt(_U('DoNotUseFertilizer'), 0xCEFD9220, 1, 1, true, 'hold', {timedeventhash = "MEDIUM_TIMED_EVENT"})
        if v.FertTimeRemove > 0 then
            while true do
                ...
                local dist = GetDistanceBetweenCoords(plcoord2.x, plcoord2.y, plcoord2.z, plantcoords.x, plantcoords.y, plantcoords.z, true)
                if dist < 2 then
                    BccUtils.Misc.DrawText3D(plantcoords.x, plantcoords.y, plantcoords.z, _U('UseFert'))
                    PromptGroup2:ShowGroup(_U('UseFert'))
                    if firstprompt2:HasCompleted() then
...
AddEventHandler('bcc-farming:WaitUntilHarvest', function(blip, timer, v, plantcoords, object, plantid)
    ...
    local firstprompt = PromptGroup:RegisterPrompt(_U('HarvestPrompt'), 0x760A9C6F, 1, 1, true, 'hold', {timedeventhash = "MEDIUM_TIMED_EVENT"})
    while true do
        Wait(5)
        local plcoord2 = GetEntityCoords(PlayerPedId())
        ------TIMER SETUP ------
        ...
        if dist < 2 then
            if timer > 0 then --if the timer is more than 0 then (if the timer hasnt reached 0 yet)
                ...
                BccUtils.Misc.DrawText3D(plantcoords.x, plantcoords.y, plantcoords.z, tostring(roundedtimer2) .. ' ' .. _U('TimerText')) --creates the text at the plants coords
            elseif timer <= 0 then --elseif the timer is less than or is 0 then (if the timer has finished/ has reached 0)
                BccUtils.Misc.DrawText3D(plantcoords.x, plantcoords.y, plantcoords.z, _U('ReadytoHarvest'))
                PromptGroup:ShowGroup(_U('HarvestPrompt'))
